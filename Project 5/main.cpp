...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("Doctor Who: Sidescroll Advendture", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL);
        ...
        switch (event.type) {
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_SPACE:
                        if (currentScene->state.player->mapBottom && (currentScene->state.player->status == 0)) {
...
void Render() {
    ...
    if (currentScene->state.player->status == 1) {
        Util::DrawText(&program, Util::LoadTexture("font.png"), "You lose!", 1.0f, -0.5f, glm::vec3(3, -3.75f, 0));
        
    }
    else if (currentScene->state.player->status == 2) {
        Util::DrawText(&program, Util::LoadTexture("font.png"), "You win!", 1.0f, -0.5f, glm::vec3(6, -3.75f, 0));
    }
    else {
